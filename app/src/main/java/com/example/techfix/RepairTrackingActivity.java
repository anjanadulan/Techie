...
public class RepairTrackingActivity extends CustomerScreen {
  ...
  private void bindAppointment() {
    ...
    CustomerRepository.AppointmentItem appointment =
        appointmentId > 0
            ? repository.getAppointment(sessionManager.getUserId(),
                                        appointmentId)
    ...
    ((TextView)findViewById(R.id.tvTrackingId))
    ...
    ((TextView)findViewById(R.id.tvTrackingDevice))
        .setText(appointment.deviceDetails);
    ((TextView)findViewById(R.id.tvTrackingService))
        .setText(appointment.serviceName);
    ((TextView)findViewById(R.id.tvTrackingEstimate))
        .setText("Appointment · " +
                 CustomerRepository.formatDateTime(appointment.appointmentAt));
    ((TextView)findViewById(R.id.tvTechnicianName))
        .setText(appointment.technicianName == null
                     ? "Awaiting assignment"
                     : appointment.technicianName);
    ((TextView)findViewById(R.id.tvTechnicianMeta))
        .setText((appointment.technicianName == null ? "TechFix team"
                                                     : "Assigned technician") +
                 " · " +
                 (appointment.branchName == null ? "Branch pending"
                                                 : appointment.branchName));
    TextView paymentButton = findViewById(R.id.btnCustomerPayment);
    CustomerRepository.PaymentItem payment =
        repository.getPayment(sessionManager.getUserId(), appointment.id);
    boolean paid = payment != null && "PAID".equals(payment.status);
    boolean ready = appointment.status == AppointmentStatus.READY_FOR_PAYMENT;
    paymentButton.setText(paid    ? "View payment receipt"
                          : ready ? "Pay repair bill"
                                  : "Payment available when repair is ready");
    paymentButton.setEnabled(paid || ready);
    paymentButton.setAlpha(paid || ready ? 1f : 0.55f);
    paymentButton.setOnClickListener(
        view -> PaymentActivity.open(this, appointment.id));
    renderTimeline(appointment);
  }

  private void renderTimeline(CustomerRepository.AppointmentItem appointment) {
    String[] titles = {"Appointment submitted", "Technician assigned",
                       "Repair in progress", "Ready for payment",
                       "Repair completed"};
    int[] markerIds = {R.id.timelineMarker1, R.id.timelineMarker2,
                       R.id.timelineMarker3, R.id.timelineMarker4,
                       R.id.timelineMarker5};
    int[] titleIds = {R.id.timelineTitle1, R.id.timelineTitle2,
                      R.id.timelineTitle3, R.id.timelineTitle4,
                      R.id.timelineTitle5};
    int[] subtitleIds = {R.id.timelineSubtitle1, R.id.timelineSubtitle2,
                         R.id.timelineSubtitle3, R.id.timelineSubtitle4,
                         R.id.timelineSubtitle5};
    int currentStage = stageFor(appointment.status);
    List<CustomerRepository.HistoryItem> history =
        repository.getRepairHistory(appointment.id);
    String latestNote = history.isEmpty()
                            ? appointment.problemDescription
                            : history.get(history.size() - 1).notes;
